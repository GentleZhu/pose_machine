...
caffe_path = '/home/qi/pose_machine/caffe/'
...
def setLayers(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=0):
    ...
    # produce data definition for deploy net
        ...
        n.data, n.tops['label'] = L.CPMData(cpmdata_param=dict(backend=1, source=data_source, batch_size=8), 
    ...
    for l in range(0, len(layername)):
        if layername[l] == 'C':
            ...
            if layername[l+1] != 'L':
                if(state == 'image'):
                    if(batchnorm == 1):
                        ...
                        n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer], 
            ...
            if deploy == False and "lmdb" not in data_source:
                n.tops['map_vec_stage%d' % stage] = L.Flatten(n.tops[last_layer])
                if stage == 1:
                    n.tops['loss_stage%d' % stage] = L.EuclideanLoss(n.tops['map_vec_stage%d' % stage], n.tops[label_name[0]])
                else:
...
def setLayers_new(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=0, lr_mult_distro=[1,1,1]):
    # it is tricky to produce the deploy prototxt file, as the data input is not from a layer, so we have to creat a workaround
    # producing training and testing prototxt files is pretty straight forward
    n = caffe.NetSpec()
    assert len(layername) == len(kernel)
    ...
    # produce data definition for deploy net
        ...
        n.data, n.tops['label'] = L.CPMData(cpmdata_param=dict(backend=1, source=data_source, batch_size=batch_size), 
                                                    ...
                                                    ntop=2)
        if exp == 28:
            n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=3), ntop=2)
        elif exp == 29:
            n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=29), ntop=2)   
        elif exp == 30 or exp == 39 or exp == 40 or exp == 41:
            n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=transform_param['num_parts']+1), ntop=2)   
        elif exp == 31:
            n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=transform_param['num_parts']), ntop=2)   
        else:
            n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=transform_param['num_parts']+1), ntop=2)   
            #n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=15), ntop=2)
        
        if exp == 31:
            n.silence = L.Silence(n.tops[label_name[1]], ntop=0)
        else:
            n.silence = L.Silence(n.tops[label_name[0]], ntop=0)
    else:
        input = "data"
        dim1 = 1
        dim2 = 4
        dim3 = transform_param['crop_size_x']
        dim4 = transform_param['crop_size_y']
        # make an empty "data" layer so the next layer accepting input will be able to take the correct blob name "data",
        # we will later have to remove this layer from the serialization string, since this is just a placeholder
        n.data = L.Layer()
    ...
    for l in range(0, len(layername)):
        if layername[l] == 'I':
            last_layer = 'conv4_4' # not used anymore from 78.3
        if layername[l] == 'V': #pretrained VGG layers
            ...
            n.tops[conv_name] = L.Convolution(n.tops[last_layer], kernel_size=kernel[l],
                                                  ...
                                                  bias_filler=dict(type='constant'))
            last_layer = conv_name
            print '%s\tch=%d\t%.1f' % (last_layer, outCH[l], lr_m)
        ...
        if layername[l] == 'C':
            ...
            #if not share_point:
            #    share_point = last_layer
            n.tops['concat_stage%d' % stage] = L.Concat(n.tops[last_layer], n.tops[share_point], concat_param=dict(axis=1))
...
def setLayers_joint(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=0, lr_mult_distro=[1,1,1]):
    # it is tricky to produce the deploy prototxt file, as the data input is not from a layer, so we have to creat a workaround
    # producing training and testing prototxt files is pretty straight forward
    n = caffe.NetSpec()
    assert len(layername) == len(kernel)
    assert len(layername) == len(stride)
    assert len(layername) == len(outCH)
    if exp == 36 or exp == 32 or exp == 37:
        num_parts = transform_param['num_parts'] - 1
    else:
    ...
    # produce data definition for deploy net
        ...
        if exp == 36 or exp == 32 or exp == 37:
            n.tops[label_name[0]], n.tops[label_name[1]], n.tops[label_name[2]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=[num_parts*2, num_parts*3+2]), ntop=3)   
        elif exp == 33:
            n.silence = L.Silence(n.tops[label_name[1]], ntop=0)
        else:
            n.tops[label_name[0]], n.tops[label_name[1]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=transform_param['num_parts']*2), ntop=2)          
    ...
    for l in range(0, len(layername)):
        ...
        if layername[l] == 'C':
            ...
            if deploy == False and "lmdb" not in data_source:
                ...
                n.tops['loss_stage%d' % stage] = L.EuclideanLoss(n.tops[last_layer[0]], n.tops[label_name[0]])
...
def writePrototxts(dataFolder, sub_dir, batch_size, layername, kernel, stride, outCH, transform_param_in, base_lr, folder_name, label_name='label_1st', batchnorm=0, lr_mult_distro=[1,1,1], new=0):
    ...
    if new == 0:
        ...
        with open('%s/pose_train_test.prototxt' % sub_dir, 'w') as f:
            ...
            str_to_write = setLayers(source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=batchnorm)
            f.write(str_to_write)

        with open('%s/pose_deploy.prototxt' % sub_dir, 'w') as f:
            print 'writing deploy prototxt'
            str_to_write = str(setLayers('', 0, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=True, batchnorm=batchnorm))
            f.write(str_to_write)
    elif new == 1:  # fine-tune 
        print 'new'
        with open('%s/pose_train_test.prototxt' % sub_dir, 'w') as f:
            print 'writing train_test prototxt'
            str_to_write = setLayers_new(source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=batchnorm, lr_mult_distro=lr_mult_distro)
            f.write(str_to_write)

        with open('%s/pose_deploy.prototxt' % sub_dir, 'w') as f:
            print 'writing deploy prototxt'
            str_to_write = str(setLayers_new('', 0, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=True, batchnorm=batchnorm, lr_mult_distro=lr_mult_distro))
            f.write(str_to_write)
    elif new == 2:  # jointly training heatmap and distance map
        print 'new'
        with open('%s/pose_train_test.prototxt' % sub_dir, 'w') as f:
            print 'writing train_test prototxt'
            str_to_write = setLayers_joint(source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=batchnorm, lr_mult_distro=lr_mult_distro)
            f.write(str_to_write)

        with open('%s/pose_deploy.prototxt' % sub_dir, 'w') as f:
            print 'writing deploy prototxt'
            str_to_write = str(setLayers_joint('', 0, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=True, batchnorm=batchnorm, lr_mult_distro=lr_mult_distro))
            f.write(str_to_write)        
...
if __name__ == "__main__":
    ...
    if(exp == 1):
        directory = '../UE4_exp_caffe/scissors1/exp3_stage3_2/'
        directory_base = 'UE4_exp_caffe/scissors1/exp3_stage3_2/'
        # serverFolder = '/home/COCO_kpt/pose33/exp2'
        base_folder = '/home/qi/pose_machine/arch/'+directory_base+'model'
        dataFolder = '/home/qi/pose_machine/UE4/lmdb_scissors1_exp3_2'
        source = '/home/qi/pose_machine/UE4/lmdb_scissors1_exp3_2'
        base_lr = 5e-5   # 4e-5
        batch_size = 8
        np = 17     # num_parts
        lr_mult_distro = [1.0, 1.0, 4.0]
        transform_param = dict(stride=8, crop_size_x=368, crop_size_y=368,
                                 target_dist=1.8 , scale_prob=1, scale_min=0.9, scale_max=1.1,
                                 max_rotate_degree=10, center_perterb_max=40, do_clahe=False,
                                 visualize=False, np_in_lmdb=17, num_parts=np, transform_body_joint=False)
        nCP = 3
        ...
        for nc in range(0,1):
            layername = ['V','V','P'] * 2  +  ['V'] * 4 + ['P']  +  ['V'] * 2 + ['C'] * 5     + ['$'] + ['C'] * 2 + ['L'] # first-stage
            kernel =    [ 3,  3,  2 ] * 2  +  [ 3 ] * 4 + [ 2 ]  +  [ 3 ] * 7                 + [ 0 ] + [ 1 ] * 2 + [ 0 ] # first-stage
            outCH =     [64]*3 + [128]* 3  +  [256] * 4 + [256]  +  [512] * 2 + [256]*4+[128] + [ 0 ] + [512] +[np+1]+[ 0 ] # first-stage
            stride =    [ 1 , 1,  2 ] * 2  +  [ 1 ] * 4 + [ 2 ]  +  [ 1 ] * 7                 + [ 0 ] + [ 1 ] * 2 + [ 0 ] # first-stage

            #if stage >= 2:
            for s in range(2, stage+1):
                layername += ['@'] + ['C'] * (7+nc)         +  ['L']
                kernel +=    [ 0 ] + [ 7 ] * (5+nc) + [1,1] +  [ 0 ]
                outCH +=     [ 0 ] + [128] * (6+nc) + [np+1]  +  [ 0 ]
                stride +=    [ 0 ] + [ 1 ] * (7+nc)         +  [ 0 ]
            ...
            label_name = ['label_1st', 'label_all']
            writePrototxts(dataFolder, sub_dir, batch_size, layername, kernel, stride, outCH, transform_param, base_lr, d_caffemodel, label_name, 0, lr_mult_distro, 1)
