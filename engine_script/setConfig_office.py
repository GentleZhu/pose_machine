...
def cubeConfig(outfile,camera_settings,keypoints=None):
	...
	config['world_center']=[277.5,140.5,39]
...
if __name__ == "__main__":
	...
	for lat in range(1,split):
		...
		for i in range(2*split):
			...
			camera_settings['location']=[x+world_center[0],y+world_center[1],z+world_center[2]]
			camera_settings['rotation']=[-lat*d_thetad,d_phid*i]
			cubeConfig(outfile,camera_settings)


	#Add top view
	camera_settings['location']=[world_center[0],world_center[1],camera_settings['radius']+world_center[2]]
